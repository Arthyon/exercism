		...
		if (_v0.$ === 'Nothing') {
			return _Utils_Tuple2(
				_Utils_update(
					player,
					{
						health: A2($elm$core$Basics$max, player.health - manaCost, 0)
					}),
				0);
		} else {
			var mana = _v0.a;
			return (_Utils_cmp(mana, manaCost) > 0) ? _Utils_Tuple2(
				_Utils_update(
					player,
					{
						mana: $elm$core$Maybe$Just(mana - manaCost)
					}),
				manaCost * 2) : _Utils_Tuple2(player, 0);
		}
	});
...
var $author$project$RolePlayingGame$revive = function (player) {
	return (player.health > 0) ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just(
		_Utils_update(
			player,
			{
				health: 100,
				mana: (player.level >= 10) ? $elm$core$Maybe$Just(100) : $elm$core$Maybe$Nothing
			}));
};
...
var pipeFilename = "/tmp/elm_test-161108.sock";
var net = require('net'),
